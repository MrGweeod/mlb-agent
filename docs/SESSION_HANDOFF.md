...
**Last Updated:** August 4, 2026 (Session 25 — root-caused and fixed the silent stall that had blocked every parlay recommendation, production and shadow both, since 2026-07-23; deployed and confirmed live)

## Current Status
✅ **OPERATIONAL — 12-DAY PARLAY-PIPELINE STALL FIXED, DEPLOYED, AND CONFIRMED LIVE (SESSION 25)**
✅ **Both production (`mlb_parlay_recommendations_v2`) and shadow (`_enriched`) pipelines had written zero rows since 2026-07-23 17:27:51 UTC.** Root cause: ~20 `statsapi.*` call sites across the live pipeline (both prod and shadow share this code) had no network timeout at all — a slow/hung MLB Stats API response could block the pipeline thread forever, silently, with no exception and no log output. Confirmed by reading the third-party `statsapi` package's source directly (its wrappers call `requests.get()` with `request_kwargs={}` — no timeout, no way to set one for most of them) and by disproving the more obvious leading hypothesis (the trend-signals loop) with a live instrumented run first. Fixed via new `src/utils/net.py::call_with_timeout()`, applied at every call site plus a 90s overall deadline on the three long per-item loops. Deployed to both Railway services; verified live via two forced full-pipeline runs completing all 8/9 steps cleanly (~3 min each) through the step that had hung every day for 12 days. Full detail below and `ARCHITECTURE_DECISIONS.md` §37.
⚠️ **Caveat, stated plainly:** neither verification run actually hung — the failure is evidently gated by real-world conditions at the moment of the call (consistent with production stalling specifically at the 9 AM/5:30 PM ET trigger windows, not an off-schedule manual trigger). The missing-timeout defect itself is 100% code-confirmed; a live hang was not personally caught mid-flight. Watch the next real scheduled run to close this out fully.
🔲 **Not yet observed post-fix: a saved parlay recommendation.** Both verification runs today built 0 parlays — for an unrelated, confirmed-legitimate reason (best 4-leg combo only reached +337/+342 odds vs. the +400 minimum), identical pre- and post-fix. The pipeline itself is fixed; whether it saves a recommendation once the pool clears +400 again hasn't been observed yet.
...
**Last Review:** August 4, 2026 (Session 25)
**System Status:** ✅ Operational — the 12-day silent parlay-pipeline stall (production and shadow both, since 2026-07-23) is root-caused, fixed, deployed, and confirmed live via two forced full-pipeline runs (Session 25, see top of this doc and `ARCHITECTURE_DECISIONS.md` §37). Sessions 22, 23, and 24's work (reference schema, dashboard, SGO billing verification, schedule cut, prop-line capture, opposing-pitcher/point-in-time-stat backfill, coverage-vs-matchup analysis) was already live as of `1db8918` (2026-07-31) — this doc's prior "uncommitted" status was stale and is corrected above. Session 21's fixes remain confirmed merged and deployed. Not yet observed: a real scheduled-run (not manual-trigger) confirmation, or a saved parlay recommendation post-fix (today's forced runs hit 0 parlays for an unrelated, confirmed-legitimate odds-threshold reason, not the stall bug).
**Next Review:** Watch the next real scheduled run (9 AM/5:30 PM ET) confirm parlays get built once the odds pool clears +400 again / watch the first live 9 AM prop-line capture run and confirm the 8/1 SGO tier downgrade tracks the ~744/month projection / re-run the coverage-vs-matchup analysis 2026-08-06 / then decide: continue to Diamond Line dashboard step 4, or return to the remaining Session 21-era queue (confirm lineup_consistency Step 5b filter live, watch leg-cap-revert performance, build the pitcher ERA rebuild, complete manual-pick end-to-end test — carried multiple sessions now)
