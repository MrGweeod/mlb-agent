...
✅ **Fix shipped (v3): totalBases/over `composite_score` is now a direct calibrated win rate** — `win_prob = 0.2483 + 0.001523 * percentile` (leg-level linear regression, r=0.0944, n=1,421), replacing the old 5-95 percentile-scaled range that implied a much higher win probability than real. `SCORER_VERSION` → `v3_2026-08-06`. `edge_percent`/`total_odds` columns widened (`NUMERIC(9,3)`/`NUMERIC(10,3)`) as a defensive overflow safety net, independent of the calibration fix. Implemented, tested (16/16 passing), committed (`e3cdc685...`), deployed (Railway `SUCCESS`). **Live-verified same day: the first post-deploy manual run produced zero totalBases/over legs in any built parlay** (composite_score for that stat/direction now caps ~40, below hits/strikeouts' typical 65-88 selected range).
⚠️ **`composite_score` audit across all three other live stat/directions found hits/over (75% of all parlay legs) carries almost no real signal.** r=0.0116 vs. real outcome (n=4,653), despite every individual component (coverage_vs_hand r=0.1148, coverage_overall r=0.1034, coverage_recent_10 r=0.0968, effective_era r=0.0785) independently correlating in the 0.08-0.11 range. Root-caused: `effective_era` (the validated exposure-weighted signal from Session 27) only populates on 161 of 4,998 hits/over legs (~3%) — the other 97% fall back to raw `pitcher_era` (r=-0.0039), diluting the real signal underneath. Split confirms it: legs using `effective_era` show composite_score r=0.0654 vs. r=0.0115 for the raw-ERA-fallback legs. **Not fixed** — likely a `src/pipelines/enrich_legs.py` coverage gap, needs its own scoping. See `ARCHITECTURE_DECISIONS.md` §40.
⚠️ **strikeouts/over: weak-but-real signal, not verified calibrated.** r=0.0713 (n=2,405) — comparable magnitude to totalBases/over's pre-fix r=0.0944. Not fixed, not urgent.
⚠️ **totalBases/under: weak but not miscalibrated.** r=0.0232 (n=6,338, vs. `coverage_overall`) — bucketed win rate tracks roughly with score (53.8%→62.5%). Unlike totalBases/over, no severe miscalibration gap. Not fixed, not urgent.
❌ **Bug found, not fixed: `edge_pct` formula doesn't match standard American-odds EV math.** `edge_pct = win_prob_pct * (combined_odds/100) - 100` is missing the `+1` term (the stake being returned on a win) that the standard formula (`win_prob_pct * (1 + combined_odds/100) - 100`) has — the two formulas disagree by `win_prob_pct` percentage points on the same bet. Exists in two duplicated places (`generate_recommendations()` in `main.py`, and inline inside `run_pipeline()` — the inline copy is what actually runs in production). Separately: `win_prob_pct` itself is `composite_score`, never sourced from SGO or validated as a calibrated probability — the hits/over finding above is direct evidence this doesn't hold for at least one stat.
❌ **Bug found, not fixed: pipeline runs can be silently mislabeled as `auto_530pm`.** `GET /api/build-parlays?refresh=true` calls `run_pipeline()` with no `source`, which falls back to an ET-hour-based label (`<11 → auto_9am`, `<14 → auto_12pm`, `else → auto_530pm`). Confirmed live 2026-08-07: a force-refresh at 14:38 ET got saved as `source='auto_530pm'`, three hours before the real 17:30 ET slot — indistinguishable from a genuine scheduled evening run in the `source` column without checking `created_at`.
✅ **Full write-up in `ARCHITECTURE_DECISIONS.md` §40** (commit `34546bce44cdfaa5a0a6a8a474a6d864793a564c`, pushed, verified against GitHub) — same facts ported into this doc and `BUILD_STATUS.md` in their own formats.
🔲 **Pending: scope/fix the `effective_era` population gap** — hits/over's dominant leg type has its real signal diluted 97% of the time by a raw-ERA fallback with ~zero correlation to outcome. See `ARCHITECTURE_DECISIONS.md` §40.
🔲 **Pending: decide on the `edge_pct` formula fix** — missing `+1` term, plus the deeper question of whether `composite_score` should ever be used as `win_prob_pct`. See `ARCHITECTURE_DECISIONS.md` §40.
🔲 **Pending: fix the `auto_530pm` mislabeling bug** in `run_pipeline()`'s no-`source` ET-hour fallback. See `ARCHITECTURE_DECISIONS.md` §40.

**Carried from Session 27 (still true, no code change this session unless noted below):**
✅ **OPERATIONAL — SCORING REDESIGN v2_2026-08-05 DEPLOYED (SESSION 27)**
✅ **`composite_score` back-tested at AUC 0.48-0.49 — no better than random.** A same-day Claude Chat investigation found real, evidence-backed fixes for some scoring components and confirmed no validated fix exists for others. Deliberate tradeoff, made explicitly by the operator: ship what's evidence-backed today and iterate live, instrumented (`scorer_version`) so results stay attributable per-component — not this project's usual shadow-before-promote discipline. Confidence levels: hits/over exposure-weighted ERA (well-validated, two rounds of testing) > totalBases/over `pt_tb_rate` (real correlation, r=0.121, but zero production track record before today) > strikeouts (no validated fix — shipped unchanged, deliberately).
✅ **totalBases (both directions) promoted to the live parlay pool** — the single highest-leverage change in this session. `over` is new to the pipeline entirely (never scored in production before); `under` had 5,000+ resolved legs of real history but was hardcoded out of parlays for shadow-only validation — that restriction is removed. On the 2026-08-05 9 AM run, 112 of 175 scored legs (64%) were totalBases legs sitting fully scored and structurally barred from parlays — the `ALLOWED_PROPS` whitelist, not the coverage gate, was the dominant bottleneck on thin-pool days.
✅ **hits/over: opposing-starter ERA/WHIP now exposure-weighted** by rolling 5-start average innings pitched (`exposure_weight = MIN(rolling_avg_ip/6.0, 1.0)`, blended with a league-average fallback). Fixes a confirmed-backwards raw-ERA signal: weak-tier starters get pulled early (4.68 IP/start vs. 5.39 for strong-tier), so the batter disproportionately faces the bullpen instead — splitting the weak tier by exposure confirmed the fix (weak+short-outing 52.8% WR vs. weak+goes-deep 60.9%, n=36/23).
✅ **hits/over coverage gate lowered 65% → 55%** — `coverage_overall` showed ~0 correlation with actual hit/miss outcomes in every test this session (r=0.007 to -0.048 across gated/ungated/production-gate reads). strikeouts/over gate left unchanged at 65% — no evidence either direction.
✅ **totalBases/over gates on sample size only** (games >= 5 in `mlb_player_batting_cumulative`, point-in-time, strictly before run date) since no `coverage_overall` has ever been computed for this direction. Scores via percentile rank of `pt_tb_rate` (total bases per game) within the day's eligible pool.
✅ **`scorer_version` (`'v2_2026-08-05'`) instrumentation** — stamped on every scored leg (`mlb_scored_legs`) and every parlay/leg produced by this pipeline (`mlb_parlay_recommendations_v2`/`mlb_parlay_legs_v2`), via a new Supabase migration (`sql/scorer_v2_2026_08_05_migration.sql`, applied directly). This is the substitute safety net for skipping the usual shadow-testing step — makes "how did this version actually perform" a clean, unambiguous query.
✅ **Pitcher strikeouts confirmed explicitly out of scope**, not an oversight: the historical resolution bug is fixed, but `src/apis/sportsgameodds.py` still normalizes `batting_strikeouts`/`pitching_strikeouts` to the same `'strikeouts'` name at ingestion, and `_find_qualifying_legs()` is architecturally batter-only (`Skip all pitchers`). Reintroducing this is a real, separate build — flagged as a follow-up, not attempted this session.
✅ **32 new unit tests, 59/59 collectible tests passing.** 3 pre-existing test files (`test_bug_fixes.py`, `test_enriched_scorer.py`, `test_lineup_confirmation.py`) can't even collect in this sandbox — confirmed via `git stash` that they fail identically against the pre-change baseline (`KeyError: 'DATABASE_URL'`, no reachable local Postgres) — a sandbox-environment gap, not a regression from this session.
✅ **Deployed to both Railway services (`mlb-agent`, `dashboard-api`), both `SUCCESS`.** Live verification of the actual run output (totalBases legs reaching the eligible pool, `effective_era` populated, `scorer_version` on real rows) is pending the next scheduled trigger (9 AM/5:30 PM ET) — this session's Railway MCP connector redacted `WEB_APP_PASSWORD`'s value, so the authenticated manual-trigger endpoint couldn't be called directly; see Pending Items.
🔲 **Pending: confirm scorer_version/totalBases/effective_era show up in a real run's output** — query `mlb_scored_legs`/`mlb_parlay_recommendations_v2` for `run_date = 2026-08-05` after the next scheduled run and compare against this session's expectations.

**Carried from Session 26 (still true, no code change this session):**
...
## What Happened on August 5, 2026 (Session 27)

### Context

A same-day Claude Chat investigation back-tested `composite_score` and found it has essentially no ranking ability (AUC 0.48-0.49, at or below random). The investigation found real, evidence-backed fixes for some scoring components (hits/over exposure-weighted ERA, totalBases promotion) and confirmed no validated fix exists for others (strikeouts). Operator decision, explicit and deliberate: ship what's evidence-backed today rather than wait for full validation on every component, with `scorer_version` instrumentation as the substitute safety net for skipping the usual shadow-before-promote discipline.

### Gate 1 changes (`main.py`, `_find_qualifying_legs()`)

- **hits/over: 65% → 55%.** `coverage_overall` showed ~0 correlation with actual hit/miss outcomes in every test this session (gated data r=0.007, ungated full-range r=-0.048, even the production gate metric itself r=0.022) — no evidence the 65% floor selected for anything outcome-relevant.
- **strikeouts/over: unchanged at 65%.** No evidence either direction — left alone deliberately.
- **totalBases/over: new sample-size-only gate** (`games >= 5` in `mlb_player_batting_cumulative`, via new `get_batter_point_in_time_totals()` in `src/utils/db.py`), since no `coverage_overall` has ever been computed for this direction. Point-in-time — strictly before the run date, same discipline as the existing `pt_*` backfill columns.
- Odds window (-250/+150) kept consistent across all three props, unchanged.

### totalBases promoted to the live pool (`main.py`)

Confirmed via direct code read (not assumption) that `ALLOWED_PROPS` in `_find_qualifying_legs()` held exactly 4 entries, with `totalBases/under` explicitly commented "shadow validation only — never enters production parlays," and `totalBases/over` entirely absent. Two changes: added `("totalBases", "over", 1.5)` as a genuinely new entry (never scored via this path before), and removed the hardcoded `production_legs = [l for l in qualifying_legs if l.get("stat") != "totalBases"]` shadow-exclusion in Step 8 (previously logged `"[N totalBases leg(s) held for shadow — excluded from parlays]"`). The player-cap fallback-restoration logic (which had to separately simulate the TB exclusion to avoid a misleading thin-pool read) simplified accordingly, since totalBases legs are now ordinary pool members. `src/engine/parlay_builder.py` needed no changes — it has no stat-specific logic at all, only `composite_score`/odds thresholds.

Evidence this was the dominant lever, not the coverage gate: on the 2026-08-05 9 AM run, the coverage/composite gate let 41 of 63 hits+strikeouts legs through (65% pass rate, not especially restrictive) — but 112 of 175 scored legs that day (64% of everything scored) were totalBases legs sitting fully scored and structurally barred from ever reaching a parlay.

### hits/over exposure-weighted ERA/WHIP (`src/pipelines/enrich_legs.py`, `src/engine/simple_scorer.py`)

New `effective_era`/`effective_whip`/`exposure_weight` fields, computed per-leg in `enrich_legs()` from a new `get_starter_rolling_ip()` point-in-time query (`src/utils/db.py`, last 5 starts strictly before run date, `mlb_player_pitching_logs WHERE is_starter=true` joined to `mlb_games`). `exposure_weight = MIN(rolling_avg_ip_last_5_starts / 6.0, 1.0)`; league-average fallback constants reused directly from `src/apis/matchup.py`'s own normalization midpoints (`_ERA_MID=4.00`, `_WHIP_MID=1.25`) rather than adding a new live aggregate query — flagged here per the task's own request to note which approach was used.

`simple_scorer.py`'s hits/over threshold logic (`pitcher_era > 5.0` / `< 3.0`) now reads `effective_era` (falling back to raw `pitcher_era` when no rolling-IP data exists yet, e.g. season-openers) — **for `over` only**; `under` is unchanged, since the validated fix was specifically for overs. One correction worth noting: the task doc referenced this replacing "`opp_pt_era`/`opp_pt_whip`" — those field names don't actually exist in the live scoring path (they're offline `mlb_training_data` backfill columns only, from `scripts/backfill_point_in_time_stats.py`); the live path uses `pitcher_era`/`pitcher_whip` from `src/apis/matchup.py`'s season-to-date fetch. Mapped the intent onto the actual field names rather than the task doc's assumed ones. WHIP is computed and attached to every leg for future analysis, but — matching the fact that raw WHIP was already removed from `simple_scorer.py` on June 25 for reliever contamination, and that this session's validated evidence covers ERA specifically — `effective_whip` isn't wired into any scoring adjustment yet.

### totalBases/over + strikeouts scoring (`src/engine/simple_scorer.py`)

totalBases/over scores via percentile rank of `pt_tb_rate` (total bases per game, from the same point-in-time `mlb_player_batting_cumulative` query used by the Gate 1 sample-size check) within the day's eligible totalBases/over pool — computed once per pipeline run in a new `_attach_totalbases_over_percentiles()`, consumed via an early-return branch in `calculate_composite_score()` that bypasses every other signal (coverage, trend, pitcher matchup, lineup stability) for this stat/direction, since none of those apply and testing showed the hits/over exposure feature specifically has no effect here (deep 56.8% vs. short 58.4%, flat/noisy). Strikeout scoring logic is byte-for-byte unchanged.

### scorer_version instrumentation (`sql/scorer_v2_2026_08_05_migration.sql`, `src/utils/db.py`, `src/engine/simple_scorer.py`)

`SCORER_VERSION = "v2_2026-08-05"` stamped on every leg inside `score_legs()`. Migration applied directly via the Supabase MCP (not just written to a file) — added `scorer_version` to `mlb_scored_legs`/`mlb_parlay_recommendations_v2`/`mlb_parlay_legs_v2`, plus `pt_tb_rate`/`effective_era`/`effective_whip`/`exposure_weight` to `mlb_scored_legs` for full traceability of the new signals, and indexes on `scorer_version` for both tables to support "how did this version perform" queries. Verified live via `information_schema.columns` query post-migration. `log_scored_legs()`/`save_parlay_recommendations_v2()` updated to persist all of it; `get_pending_lineup_legs()` (the lineup-recheck re-scoring path) updated to select `scorer_version` too so it survives a re-save rather than silently dropping to NULL.

### Testing

32 new unit tests across three new files: `tests/test_main_gates_2026_08_05.py` (the 55%/65%/sample-size gates, via a fully-stubbed `main.py` import), `tests/test_simple_scorer_2026_08_05.py` (effective_era usage, percentile scoring, scorer_version stamping, strikeouts-unchanged — pure functions, no stubbing needed), `tests/test_db_point_in_time_2026_08_05.py` (the two new point-in-time query helpers, with `psycopg2.connect` mocked so `db.py`'s import-time `init_db()` never touches a real connection). Found and fixed a real test-isolation bug in the process: an early version of the `main.py` gate tests left `sys.modules` stubs for `src.engine.parlay_builder`/`src.utils.db` uncleaned, which silently broke `test_parlay_builder.py` (15 failures) depending on collection order — fixed by recording and restoring prior `sys.modules` state after `import main` completes, verified stable by running the suite in multiple orders.

Full suite: 59/59 collectible tests passing. 3 pre-existing files (`test_bug_fixes.py`, `test_enriched_scorer.py`, `test_lineup_confirmation.py`) fail to even collect in this sandbox (`KeyError: 'DATABASE_URL'` — no `.env` here, and `db.py` calls `init_db()` unconditionally at import time). Confirmed via `git stash` that this is identical against the pre-change baseline, not a regression introduced this session.

### Deploy

Committed (`de9eae3`) and pushed to `origin/master`. Both Railway services (`mlb-agent`, `dashboard-api`) redeployed successfully (`SUCCESS`, confirmed via `list-deployments`). Live-run verification (totalBases legs reaching the eligible pool, `effective_era` distinct from raw values, `scorer_version` present, no pitcher-role strikeout props) is pending the next scheduled trigger — this session's Railway MCP connector redacted `WEB_APP_PASSWORD`'s actual value (variable names only, not values, for this OAuth connection), so the authenticated `POST /api/admin/run_pipeline` endpoint couldn't be called directly from this session. See Pending Items.

### Session 27 status

Implemented, unit-tested, migrated, and deployed. Live-run verification against real production data is the one open item, blocked on either the next scheduled run (9 AM/5:30 PM ET) or the operator supplying `WEB_APP_PASSWORD` directly. New/changed files: `main.py`, `src/engine/simple_scorer.py`, `src/pipelines/enrich_legs.py`, `src/utils/db.py`, `sql/scorer_v2_2026_08_05_migration.sql` (new), `tests/test_main_gates_2026_08_05.py`/`test_simple_scorer_2026_08_05.py`/`test_db_point_in_time_2026_08_05.py` (new), plus this doc, `BUILD_STATUS.md`, `ARCHITECTURE_DECISIONS.md`.
...
### 0. Confirm the scoring redesign's real-run output (High Priority, new Session 27)
Deployed and unit-tested but not yet confirmed against a real pipeline run's actual output. After the next scheduled 9 AM/5:30 PM ET run (or a manually-triggered one, if `WEB_APP_PASSWORD` is available), query directly: `SELECT stat, direction, COUNT(*), COUNT(scorer_version) FROM mlb_scored_legs WHERE run_date = '2026-08-05' GROUP BY 1,2` to confirm totalBases/over legs are present with `pt_tb_rate` set, hits/over legs show `effective_era` distinct from raw `pitcher_era` where exposure differs, strikeouts legs are unaffected, and no pitcher-position strikeout legs appear. Also confirm `mlb_parlay_recommendations_v2` includes at least one totalBases leg where eligible, and check whether the widened pool actually fixes a thin-pool day (the 2026-08-05 9 AM run failed with only 148 attempts, best +315 — it's reasonable to expect improvement, not safe to assume it without checking).
...
**Last Review:** August 7, 2026 (Session 28)
**System Status:** ✅ Operational — the totalBases/over win-rate calibration fix (v3, Session 28, `ARCHITECTURE_DECISIONS.md` §40) is root-caused, fixed, deployed, and confirmed live: the numeric-overflow bug that silently zeroed out 8/5-8/6's automated parlay saves is closed, and the first post-deploy run correctly produced zero over-scored totalBases/over legs. Four items opened this session remain unresolved: the hits/over `effective_era` population gap (dominant leg type, real signal diluted 97% of the time by a near-zero-correlation ERA fallback), the `edge_pct` formula bug (missing `+1` term, plus `win_prob_pct` never being a validated probability), the `auto_530pm` mislabeling bug, and two logged-but-not-urgent weak-signal findings (totalBases/under, strikeouts/over). Everything carried from Sessions 21-27 (parlay-builder floor-recovery, pipeline-stall fix, reference-data/dashboard work, prop-line capture, point-in-time backfill, leg-scoring redesign v2) remains fixed, deployed, and confirmed live as previously documented — unchanged this session.
**Next Review:** Decide scope/priority on the hits/over `effective_era` population gap (§40 — likely `src/pipelines/enrich_legs.py`) / decide on the `edge_pct` formula fix (§40 — missing `+1` term and the deeper composite_score-as-probability question) / scope a fix for the `auto_530pm` mislabeling bug (§40) / revisit the two logged-but-not-urgent weak-signal findings when time allows (§40 — totalBases/under r=0.0232, strikeouts/over r=0.0713) / still carried: watch the next real scheduled run (9 AM/5:30 PM ET) confirm parlays get built and saved without any manual trigger, watch the first live 9 AM prop-line capture run and confirm the 8/1 SGO tier downgrade tracks the ~744/month projection, re-run the coverage-vs-matchup analysis, and the remaining Session 21-era queue (confirm lineup_consistency Step 5b filter live, watch leg-cap-revert performance, build the pitcher ERA rebuild, complete manual-pick end-to-end test)
**Pending Decisions:** Scope/priority for the four new Session 28 open items above (effective_era gap, edge_pct formula, auto_530pm mislabeling, two weak-signal findings), whether to continue straight to dashboard step 4 or the older queue first (operator's call), whether/when to root-cause the mlb_scored_legs game_pk mismatch (Session 22), whether the `'midday'`/`'evening'` source-labeling gap and the dormant `get_totals_props()` bug are worth fixing (both Session 23, both low priority/pre-existing), Pitcher ERA rebuild scope and timeline (scoped Session 21, not started), whether/how to promote totalBases to production given the coverage-vs-matchup analysis's supportive finding for that bet type (Session 24, operator's call), SGO Pro cancellation (user action, deadline 2026-08-01)
