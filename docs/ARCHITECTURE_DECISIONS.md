...
**Last Updated:** July 30, 2026 (Session 24 — opposing-pitcher ID capture fixed forward into `mlb_training_data`; point-in-time batter/pitcher stat backfill built and wired into the daily job; coverage-vs-matchup analysis completed, re-run scoped for 8/6/2026)
...
38. [Future Considerations](#future-considerations)
...
**Architecture Status:** ✅ STABLE — Session 21's fixes confirmed merged and deployed (verified via `git merge-base --is-ancestor` this session — corrects the stale "pending merge" status this footer carried since Session 21 without being re-checked). Sessions 22, 23, and 24's combined work is validated and sitting uncommitted as of this update — see `SESSION_HANDOFF.md`/`BUILD_STATUS.md` for commit status.
**Last Major Change:** July 30, 2026 (Session 24) — `opposing_pitcher_id` capture fixed forward into `mlb_training_data` (§35); point-in-time batter/pitcher stat backfill (`mlb_player_batting_cumulative`/`_pitching_cumulative`, `pt_*`/`opp_pt_*` columns) built, wired into the daily job, and run once against production; coverage-threshold-vs-matchup-quality analysis completed, re-run scheduled 2026-08-06 once `mlb_prop_legs_history` has a week of ungated data (§36)
**Prior Major Change:** July 29, 2026 (Sessions 22+23) — reference-data schema backfilled season-to-date; Diamond Line dashboard Standings/Leaderboards pages built; SGO billing confirmed per-event; pipeline schedule cut 3→2 runs/day; full prop-line capture + resolution built into `mlb_prop_legs_history`
