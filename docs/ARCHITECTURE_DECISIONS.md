...
## Leg Scoring Redesign v2_2026-08-05 — Ship Evidence-Backed Fixes Per-Component, Instrumented (Session 27)

### **Decision: Skip Full Shadow-Validation This Once, Deliberately, With scorer_version as the Substitute Safety Net**

**Background.** `composite_score` was back-tested and found to have essentially no ranking ability (AUC 0.48-0.49, at or below random). A same-day Claude Chat investigation found real, evidence-backed fixes for some components and confirmed no validated fix exists for others. This project's usual discipline is shadow-before-promote (see the enriched-scorer shadow pipeline, `mlb_prop_legs_history`'s ground-up-rebuild isolation). The operator explicitly chose to deviate from it here: ship what's evidence-backed today and iterate live rather than wait for full validation on every component — with the requirement that `scorer_version` instrumentation make "how did this version actually perform" a clean, unambiguous query, as the substitute for the validation step being skipped.

**Confidence levels, treated differently in the implementation, not uniformly:**
- hits/over exposure-weighted ERA: well-validated (two rounds of live-data testing, holding batter coverage constant)
- totalBases/over `pt_tb_rate`: real correlation (r=0.121, 5-fold CV ~0.57 AUC) but zero production track record — first live pass
- strikeouts: no validated improvement exists — shipped unchanged, deliberately, not by oversight

### Gate 1 changes

hits/over lowered 65%→55%: `coverage_overall` showed ~0 correlation with actual hit/miss outcomes in every test this session (gated data r=0.007, ungated full-range r=-0.048, even the production gate metric itself r=0.022) — no evidence the 65% floor selected for anything outcome-relevant. strikeouts/over left unchanged at 65% — no evidence either direction, don't guess. totalBases/over gated on sample size only (`games >= 5` in `mlb_player_batting_cumulative`, new `get_batter_point_in_time_totals()` in `src/utils/db.py`) rather than `coverage_overall`, since that signal has never been computed for this direction — `_hitter_coverage()`'s own `get_season_minimum()` gate (8-20 games) would have applied a much stricter, unintended bar if this stat/direction had simply been routed through the normal `calculate_coverage()` gating path like everything else.

### totalBases promoted to the live pool — the dominant lever, confirmed by reading the code, not assumed

Corrected an earlier assumption (from the task's own framing) that totalBases/over was merely shadow-flagged. Direct code read of `_find_qualifying_legs()`'s `ALLOWED_PROPS` found exactly 4 entries, with `("totalBases", "under", 1.5)` commented "shadow validation only — never enters production parlays," and `over` entirely absent — never scored via this path at all. Two changes: added `("totalBases", "over", 1.5)` as a genuinely new entry, and removed the hardcoded `production_legs = [l for l in qualifying_legs if l.get("stat") != "totalBases"]` exclusion in Step 8 (`main.py`) that had been holding `under` out of parlays despite 5,000+ resolved legs of real production scoring history. `src/engine/parlay_builder.py` needed zero changes — confirmed via full read that it has no stat-specific logic anywhere, only `composite_score`/odds thresholds, so any stat flows through identically once it reaches the pool.

Evidence this was the dominant lever, not the coverage/gate threshold change above: on the 2026-08-05 9 AM run, the coverage/composite gate let 41 of 63 hits+strikeouts legs through (65% pass rate — not especially restrictive), while 112 of 175 scored legs that day (64% of everything scored) were totalBases legs sitting fully scored and structurally barred from ever reaching a parlay. Widening a whitelist that excludes 64% of scored volume outweighs adjusting a gate that was already passing 65% of what reached it.

### hits/over exposure-weighted ERA/WHIP — and a task-doc assumption that didn't match the live code

**Confirmed via live-data testing, holding batter coverage constant, before implementing.** Raw `pitcher_era` produced a backwards signal for hits/over: legs vs. "weak" pitchers (ERA-bucketed) won at 57.9%, worse than vs. "strong" pitchers at 64.9% (n=95/148). Root cause: weak-tier starters threw 4.68 innings/start on average vs. 5.39 for strong-tier — they get pulled early, so the batter disproportionately faces the bullpen instead of the (weak) starter the leg was scored against. Splitting the weak tier further by exposure confirmed the fix: weak+short-outing legs won at 52.8% vs. weak+goes-deep at 60.9% (n=36/23).

**Implementation:** `exposure_weight = MIN(rolling_avg_ip_last_5_starts / 6.0, 1.0)`; `effective_era = era * exposure_weight + league_avg_era * (1 - exposure_weight)` (same formula for WHIP). `rolling_avg_ip_last_5_starts` comes from a new `get_starter_rolling_ip()` in `src/utils/db.py` — the starter's last 5 starts strictly before the run date, `mlb_player_pitching_logs WHERE is_starter = true` joined to `mlb_games` for `game_date`, point-in-time, no lookahead. League-average fallback constants (`_ERA_MID=4.00`, `_WHIP_MID=1.25`) reused directly from `src/apis/matchup.py`'s own normalization midpoints rather than adding a new live aggregate query — noting the approach explicitly, per the task's own request, since a fallback constant and a live aggregate are meaningfully different choices worth being able to audit later.

**A discrepancy between the task doc and the actual live code, resolved by reading the code rather than the doc.** The task instructions referenced replacing "raw `opp_pt_era`/`opp_pt_whip` in `simple_scorer.py`'s existing hits/over bucket logic." Neither field exists in the live scoring path — `opp_pt_era`/`opp_pt_whip` are offline `mlb_training_data` backfill columns only (`scripts/backfill_point_in_time_stats.py`), computed once per historical row, never read by `main.py` or `simple_scorer.py`. The live path uses `pitcher_era`/`pitcher_whip`/`pitcher_k9`, fetched season-to-date via `src/apis/matchup.py::get_pitcher_matchup_profile()` and attached per-leg in `enrich_legs.py`. Also, `simple_scorer.py`'s hits/over logic is a flat threshold check (`era > 5.0` / `< 3.0`), not "bucket logic" in any more elaborate sense, and raw WHIP was already removed from it on June 25 (reliever contamination in the full-season rank pool — see the in-file comment). Implemented the actual intent (blend by exposure) against the actual field names and actual code shape, rather than the task doc's assumed ones — `effective_whip` is computed and attached to every leg (available for future analysis/instrumentation) but not wired into any scoring adjustment yet, since reintroducing a WHIP-based signal isn't what this session's validated evidence covers, and doing so anyway would work against the "don't uniformly treat these as equally proven" instruction this task itself set.

`effective_era` is consulted for `hits/over` only, with a fallback to raw `pitcher_era` when no rolling-IP data exists yet (season-openers). `hits/under` is unchanged, deliberately — the validated exposure fix covers overs only.

### totalBases/over scoring — percentile rank, not a black-box model

Scores primarily off `pt_tb_rate` (batter's point-in-time total-bases-per-game rate, same `mlb_player_batting_cumulative` query used for the Gate 1 sample-size check — `total_bases / games` as of the most recent row strictly before the run date). Implemented as a percentile rank within that day's eligible totalBases/over pool (`_attach_totalbases_over_percentiles()` in `simple_scorer.py`, computed once per pipeline run before the per-leg scoring loop), matching the project's existing transparent, inspectable scoring style rather than a black-box model. `calculate_composite_score()` early-returns for this stat/direction, bypassing every other signal (coverage, trend, pitcher matchup, lineup stability) — none of those apply, and testing found the hits/over exposure feature specifically has no effect here (deep 56.8% vs. short 58.4% WR, flat/noisy), so it wasn't ported over.

### scorer_version — the substitute safety net

`SCORER_VERSION = "v2_2026-08-05"` stamped on every leg inside `score_legs()` (`simple_scorer.py`), persisted via `log_scored_legs()`/`save_parlay_recommendations_v2()` to `mlb_scored_legs`/`mlb_parlay_recommendations_v2`/`mlb_parlay_legs_v2`. Migration (`sql/scorer_v2_2026_08_05_migration.sql`) applied directly against production via the Supabase MCP rather than left as a file to run manually — also added `pt_tb_rate`/`effective_era`/`effective_whip`/`exposure_weight` to `mlb_scored_legs` for full traceability of the new signals (not just the version tag), and an index on `scorer_version` for both `mlb_scored_legs` and `mlb_parlay_recommendations_v2` to keep "how did this version perform" queries cheap. Verified live via `information_schema.columns`, not assumed from the migration's exit status. `get_pending_lineup_legs()` (the lineup-recheck re-scoring path) updated to select `scorer_version` too, so a leg re-saved through that path doesn't silently lose its version attribution.

### Testing — a real cross-test contamination bug found and fixed before trusting the suite

32 new unit tests. `tests/test_main_gates_2026_08_05.py` needed to stub `main.py`'s entire heavy import chain (statsapi, live API clients, `src.utils.db`) to test the gate logic without network/DB access — the same pattern `test_bug_fixes.py`/`test_lineup_confirmation.py` already use for `src.engine.parlay_builder`. The first version left those stubs in `sys.modules` uncleaned after use; since `sys.modules` is process-global, this silently broke `test_parlay_builder.py` (15 failures — `build_parlays()` and `_attempt_swap_recovery()` resolved to the leftover `MagicMock` stub instead of the real module) depending on collection order. Fixed by recording each stubbed module's prior `sys.modules` value before insertion and restoring it once `import main` completes (main.py has already bound its own copies of every name it imported by that point, so the stub doesn't need to remain cached). Verified stable by running the full suite in multiple different file orders.

Full suite: 59/59 collectible tests passing. 3 pre-existing files (`test_bug_fixes.py`, `test_enriched_scorer.py`, `test_lineup_confirmation.py`) fail to even collect in this session's sandbox (`KeyError: 'DATABASE_URL'` — `src/utils/db.py` calls `init_db()` unconditionally at import time, and no `.env` exists in this checkout). Confirmed via `git stash` (reverting to the pre-session commit, re-running, then restoring) that this is identical against the baseline — a sandbox-environment gap, not a regression introduced this session.

### Pitcher strikeouts — confirmed out of scope, not an oversight

The historical resolution bug (`PARLAY_RESOLUTION_BUG_POSTMORTEM.md`, May 19 2026) that voided all pitcher-K props incorrectly is confirmed fixed. A separate, still-open problem exists at ingestion: `src/apis/sportsgameodds.py` normalizes both `batting_strikeouts` and `pitching_strikeouts` raw SGO stat IDs to the same internal `'strikeouts'` name, discarding which one matched — the pipeline cannot currently distinguish a pitcher's own K prop from a batter's K-against prop at all. `_find_qualifying_legs()` also explicitly skips all pitchers as subjects — the qualifying-leg pipeline is architecturally batter-only. Reintroducing pitcher strikeouts is a real, separate build (likely needing the same isolated-path disambiguation approach `prop_legs_capture.py` already uses for this exact problem), not a whitelist entry — flagged as a follow-up, not attempted this session.

**Status: implemented, unit-tested (59/59 collectible passing), migrated live, deployed to both Railway services (confirmed `SUCCESS`). Live-run verification against real production output (totalBases legs reaching the eligible pool, `effective_era` distinct from raw values, `scorer_version` on real rows, no pitcher-role strikeout legs) is pending the next scheduled 9 AM/5:30 PM ET trigger** — this session's Railway MCP connector redacted `WEB_APP_PASSWORD`'s value (variable names only for this OAuth connection), so the authenticated manual-trigger endpoint couldn't be called directly. See `SESSION_HANDOFF.md` Pending Items §0.
...
**Revised 2026-08-12 — constrained 4-leg search added; and the problem was mostly a gated-pool artifact.**

Greedy top-4-then-check-the-floor is not a search — it inspects exactly one combination. On the 2026-08-11 *gated* pool the top 4 by `p_hit` multiplied to only +351, so the builder extended to 5 legs, while 61 of the 121 legs were priced long enough (decimal ≥ 1.495) that some four of them clear +400. Stage 1 is now an **exact branch-and-bound search** for the 4-leg combination maximising joint probability subject to +400 and every roster constraint; the greedy 5/6-leg extension is preserved as the fallback and fires only when no 4-leg solution exists. The search is verified against brute force over all C(14,4) combinations — it finds the true optimum, not a good one — and runs in <0.01s on 121 legs.

**THE FINDING WORTH NOT REDISCOVERING: the "can't find a 4-leg parlay" problem was substantially an artifact of the gated pool, and v4 removes the gate.** The gated pool for 2026-08-11 held 121 hits/over legs with decimal odds topping out at **1.926**. The real ungated pool for the same slate (from `mlb_prop_legs_history`, which records every offered line rather than only gate survivors) held **246** legs, **197** after Gate 2's odds band, with price extending to decimal **3.05**. On that pool **the top 4 by `p_hit` already clear +400 (+577)**, so the constrained search and greedy produce an identical parlay #1. The search still improves parlays #2 and #3 (4 legs at +404/+412, joint 0.277/0.253, versus greedy's 5 legs at +634/+627, joint 0.199/0.177). So: keep the search — it is strictly better or equal and costs nothing — but do not expect it to be load-bearing once the gate is gone. If a future session sees 5-leg parlays and reaches for the builder, check the pool's price distribution first.

`V4_QUALITY_FLOOR_MODE`/`_VALUE` gate which legs enter the search. **Measured, the floor is nearly inert:** every percentile from 0 to 75 produced an *identical* top parlay on both pools, because maximising the product of `p_hit` already prefers high-probability legs — the objective does the optimising, not the floor. Only p90 differed, and only by starving parlay #3 into the greedy path. `max_drop` is the more fragile parameterisation: 0.02 left 2 eligible legs and 0.05 left 7, both starving the search into exactly the greedy behaviour the change exists to avoid. Hence percentile mode at **25** — a guard rail against the search reaching into the bad tail (the ungated pool runs down to `p_hit` 0.32) positioned where it never binds on the optimum. Recorded plainly rather than dressed up: this parameter does not currently matter.

Two sharp edges found while testing, both pre-existing: legs priced outside `[-250, +150]` are dropped by `_filter_legs()` *before* any search runs (so a test fixture at −300 silently tests nothing), and `build_parlays()` caches decimal odds on the leg dict as `_dec`, so copying a leg and changing `best_odds` keeps the old price. Also worth knowing: inside the odds band six legs always clear +400 (1.4⁶ = 7.53), so infeasibility can only come from pool size or roster constraints, never from the odds floor alone.
