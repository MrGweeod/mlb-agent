...
Six steps, reusing the exact same tested functions as the one-time backfill:
  ...
  2. Cumulative batting/pitching stat refresh for YESTERDAY — appends one
     row per player who played yesterday to mlb_player_batting_cumulative /
     mlb_player_pitching_cumulative (running season-to-date totals). Must
     run after step 1 (reads the game logs just backfilled) and before step
     3. See scripts/backfill_point_in_time_stats.py's
     refresh_cumulative_tables().
  3. Point-in-time backfill for mlb_training_data — fills pt_role/pt_*/
     opp_pt_* using the cumulative tables just refreshed in step 2. Not
     date-scoped: also catches up any historical backlog (rows with
     opp_pitcher_id set but opp_pt_era still NULL). See
     scripts/backfill_point_in_time_stats.py's
     backfill_training_data_point_in_time().
  4. Resolve mlb_prop_legs_history against the game logs just backfilled in
  ...
  5. Season-stats snapshot (Qualified Players) as of TODAY — re-pulling
  ...
  6. Standings + splits snapshot as of TODAY.
...
from scripts.backfill_point_in_time_stats import (
    refresh_cumulative_tables,
    backfill_training_data_point_in_time,
)
...
def run_daily_reference_refresh(dry_run: bool = False) -> dict:
    ...
    summary = {"date": today.isoformat(), "games_backfilled": None, "prop_legs_resolved": None,
               ...
               "cumulative_refresh": None, "training_data_backfill": None}
    ...
    try:
        ...
        summary["cumulative_refresh"] = refresh_cumulative_tables(cur, yesterday)
        summary["training_data_backfill"] = backfill_training_data_point_in_time(cur)
